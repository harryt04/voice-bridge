...
export const AacPhraseInputSchema = z.object({
  ...
  text: z.string().min(1, 'Phrase text required').max(200, 'Phrase text must be 200 characters or less'),
...
export const AacPreferencesInputSchema = z.object({
  ...
  speechRate: z.number().min(0.5, 'Speech rate must be at least 0.5').max(2, 'Speech rate must be at most 2'),
  speechPitch: z.number().min(0.5, 'Speech pitch must be at least 0.5').max(2, 'Speech pitch must be at most 2'),
  ...
  mobileGridColumns: z.enum(['2', '3', '4']).transform(v => parseInt(v) as 2 | 3 | 4),
})
