...
describe('/api/vocabWords', () => {
  ...
  it('returns 401 when unauthenticated', async () => {
    ...
    const req = new NextRequest('http://localhost/api/vocabWords?speakerId=x')
  ...
  it('returns only vocab words belonging to the requested speaker', async () => {
    ...
    const client = await getMongoClient()
    const db = client.db(mongoDBConfig.dbName)
    await db.collection(mongoDBConfig.collections.vocabWords).insertMany([
      { word: 'hello', speakerId: speakerId.toString() },
      { word: 'goodbye', speakerId: otherSpeakerId.toString() },
    ])

    const req = new NextRequest(
      `http://localhost/api/vocabWords?speakerId=${speakerId.toString()}`,
    )
    const res = await GET(req)
  ...
  it("blocks listing vocab words for a speaker the user doesn't own", async () => {
