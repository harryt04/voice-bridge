...
describe('/api/activities', () => {
  ...
  it('returns 401 when unauthenticated', async () => {
    ...
    const req = new NextRequest('http://localhost/api/activities?speakerId=x')
  ...
  it('returns only activities belonging to the requested speaker', async () => {
    ...
    const client = await getMongoClient()
    const db = client.db(mongoDBConfig.dbName)
    await db.collection(mongoDBConfig.collections.activities).insertMany([
      { name: 'Swimming', speakerId: speakerId.toString() },
      { name: 'Painting', speakerId: otherSpeakerId.toString() },
    ])

    const req = new NextRequest(
      `http://localhost/api/activities?speakerId=${speakerId.toString()}`,
    )
    const res = await GET(req)
    ...
    expect(data[0].name).toBe('Swimming')
  })

  it("blocks listing activities for a speaker the user doesn't own", async () => {
