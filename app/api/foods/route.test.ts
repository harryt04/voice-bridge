...
describe('/api/foods', () => {
  ...
  it('returns 401 when unauthenticated', async () => {
    ...
    const req = new NextRequest('http://localhost/api/foods?speakerId=x')
  ...
  it('returns only foods belonging to the requested speaker', async () => {
    ...
    const client = await getMongoClient()
    const db = client.db(mongoDBConfig.dbName)
    await db.collection(mongoDBConfig.collections.foods).insertMany([
      { name: 'Apple', speakerId: speakerId.toString() },
      { name: 'Banana', speakerId: otherSpeakerId.toString() },
    ])

    const req = new NextRequest(
      `http://localhost/api/foods?speakerId=${speakerId.toString()}`,
    )
    const res = await GET(req)
  ...
  it("blocks listing foods for a speaker the user doesn't own", async () => {
