...
describe('/api/villager', () => {
  ...
  it('GET returns 401 when unauthenticated', async () => {
    ...
    const req = new NextRequest(
      `http://localhost/api/villager?id=${new ObjectId().toString()}`,
    )
  ...
  it('creates, reads, and deletes a villager scoped to the collection', async () => {
    mockGetSession.mockResolvedValue({ user: { id: 'owner-user' } })
    const speakerId = await insertSpeaker()

    const createReq = new NextRequest('http://localhost/api/villager', {
      method: 'POST',
      body: JSON.stringify({
        name: 'Grandma',
    ...
    const inserted = await db
      .collection(mongoDBConfig.collections.villagers)
      .findOne({ speakerId: speakerId.toString() })
    expect(inserted).not.toBeNull()

    const getReq = new NextRequest(
      `http://localhost/api/villager?id=${inserted!._id.toString()}`,
    )
    const getRes = await GET(getReq)
    expect(getRes.status).toBe(200)
    const fetched = await getRes.json()
    expect(fetched.name).toBe('Grandma')
